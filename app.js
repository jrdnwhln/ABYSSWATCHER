...
function buildLunaReply(message) {
  ...
  if (/(where is research|open research|articles real)/.test(lower)) {
    return "Luna says the research page uses NCBI PubMed records for health topics like fitness, cancer, viruses, parasites, and longevity.";
  }
  ...
  if (realm === "research") {
    return "Luna says this is the research realm. Search health topics, switch between latest, reviews, and clinical, and use Quick Preview for abstracts when available.";
  }
  if (realm === "caveman") {
...
function buildPubMedQuery(query, tab) {
  ...
  const baseQuery = `(${normalizedQuery}) AND ${scope} AND journal article[Publication Type]`;
  if (tab === "reviews") {
    return `(${normalizedQuery}) AND ${scope} AND review[Publication Type]`;
  }

  if (tab === "clinical") {
    return `(${normalizedQuery}) AND ${scope} AND (clinical trial[Publication Type] OR randomized controlled trial[Publication Type])`;
  }
